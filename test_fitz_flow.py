...
def _extract_words_smart_layout_fitz(page_width, fitz_words, line_tolerance=8):
    ...
    # Dynamically find true gutter center
    ...
    for w in words:
        if w['top'] <= current_bottom + line_tolerance:
    ...
    # 2. Determine type of line: 'spanning' vs 'split' (2-column)
    typed_lines = []
    for line in lines:
        ...
        w_type = 'split'
        # If the line as a whole spans the center
    ...
    # 3. Group consecutive lines of same type into blocks
    blocks = []
    ...
    # 4. Extract words logically
    final_words = []
    for block in blocks:
        if block['type'] == 'spanning':
            b_words = [w for line in block['lines'] for w in line]
            b_words.sort(key=lambda w: (w['top'] // line_tolerance, w['x0']))
